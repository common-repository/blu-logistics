 ...
 * @author    WooThemes
 */
class WC_Blu_Global_Delivery_Shipping_Method extends WC_Shipping_Method {

  /**
   * Constructor.
   */
  public function __construct(  ) {
    $this->id = 'blu_global_shipping_method';
    $this->method_title = __( 'bluGlobal', 'woocommerce' );
  ...
  public function init_form_fields() {
    $this->form_fields = array(
      'enabled' => array(
          ...
          'type'      => 'checkbox',
          'label'     => __( 'Enable bluGlobal', 'woocommerce' ),
          'default'     => 'no'
        ),
        'title' => array(
          ...
          'default'   => __( 'bluGlobal', 'woocommerce' ),
        ...
        'description' => array(
                            ...
                            'default'   => __( 'Enjoy affordable international shipping within 7-10 business days*', 'woocommerce' ),
            ),
        'tracking_url' => array(
        ...
        function generate_table_rates_table_html() {
          ...
          <script>
            ...
            */
            ...
            jQuery(zoneID).on('click', '.add-zone-buttons a.add', function(){
              ...
              if(jQuery('#woocommerce_blu_global_shipping_method_free_shipping').is(':checked'))
              {
          ...
          function create_select_arrays(){
              ...
              foreach ( $continents as $continent_code => $continent ) {
                    $this->country_array["zone:".$continent_code] = $continent['name'];
                    $countries = $continent['countries'];

                    foreach ($countries as $value) {
                        $this->country_array[$value] = ' &nbsp;&nbsp;'.$allowed_countries[$value];
                    }
              }
        ...
        */
        ...
        public function is_available( $package ){
                ...
                $is_available = $this->allow_shipping_method($package);
                return $is_available['allowed_countries'];
        }

        /**
         * calculate_shipping function.
         * Calculate blu global shipping.
         */
        public function calculate_shipping( $package = array() ) {
                ...
                if($is_available['allowed_countries']){
                    $cost = $is_available['rate'];
                }
        ...
        /**
        * find_matching_rate
        * return rate after matching max-min 
        **/
        ...
        function get_rates_for_country($country){
            ...
            $zone = 'zone:'.WC()->countries->get_continent_code_for_country( $country );

            foreach($this->options as $rate){
              if(in_array($country, $rate['countries']) || in_array($zone, $rate['countries'])){
                $ret[] =  $rate;
              }
        ...
        * get bluAPI pricing
        **/
        function getAPIPrice($package){
            $BluApi = new BluAPI();
            if($package['destination']['state'])
              $state = WC()->countries->states[$package['destination']['country']][$package['destination']['state']];
            else
              $state = $package['destination']['city'];
            $weight = WC()->cart->cart_contents_weight;

            $response = $BluApi->blsmGetAPIRate($weight, $package['destination']['city'], $package['destination']['postcode'], $state, WC()->countries->countries[$package['destination']['country']]);
            return $response;
        }

        /**
        * allow_shipping_method function
        * check whether price available for selected country
        **/
        function allow_shipping_method($package){
            $response['allowed_countries'] = false;
            $response['rate'] = 0;
            if ('no' == $this->enabled) {
                return $response;
            }
            ...
            if($options['use_api'])
            {
              $result = $this->getAPIPrice($package);
            ...
            if($options['use_api'])
            {
              ...
              if($result == "FAILED"){
                  ...
                  if($options['mark_up'] == 'fixed'){
                    $cost =  $result + $options['price'];
                  }
                  else if($options['mark_up'] == 'percent'){
                    $cost = (($options['price']/100) * WC()->cart->cart_contents_total) + $result;
                  }
                  else
                    $cost = $result;
              }

              $response['allowed_countries'] = true;
              $response['rate'] = $cost;
              return $response;

            }
            else if($this->get_option('free_shipping') == 'yes'){
                $response['allowed_countries'] = true;
                return $response;
            } 
            else {
